...
const ENERGY_DRAIN_BASE = 0.8;    // Watts/s base drain
...
export const useGameStore = create<GameState>((set, get) => ({
  ...
  tick: (deltaMs: number) => {
    ...
    // Energy drain: base + buildings consumption
    ...
    const totalDrain = (ENERGY_DRAIN_BASE + buildingDrain) / cableFactor;

    const newEnergy = Math.max(0, s.energy - totalDrain * dt);

    // Logic drain: passive drain, accelerated if energy < 20
    ...
    if (newEnergy <= 0 && newLogic <= 0) {
      set({ energy: 0, logic: 0, isGameOver: true, isGlitching: true });
      return;
    }
